...
| Brand | `Brand/` — the mark on its own, SVG and PNG. Outside `Resources/` on purpose: that directory is copied into the bundle wholesale. |
...
The mark is two capsules, one solid and one a ring, interlocked on the
diagonal — an application routed into an output. It is drawn flat in two
colours and nothing else: `Scheme` in that file carries `black`, `white`
and `orange`, and the `scheme` constant below it decides which one ships.
Two flat colours is the whole constraint, because the mark has to survive
being a favicon, a stencil, and somebody else's slide deck.

The menu bar glyph carries its own proportions rather than a scaled copy
of the icon's: at 16 pt the wall that looks elegant at 512 pt is a third
of a pixel wide, and the break at the crossing closes up entirely.
